document.addEventListener('DOMContentLoaded', function() {
  ...
  filterButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      galleryItems.forEach((item, index) => {
        ...
        item.offsetHeight; // reflow hack to restart animation
  ...
  audioTabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      /* Animate each .audio-card inside the newly activated tab */
      const audioCards = activeContent.querySelectorAll('.audio-card');
      audioCards.forEach((card, index) => {
        card.style.animation = 'none';
        card.offsetHeight; // reflow to reset
